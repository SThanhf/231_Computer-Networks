...
{                                         // no guarantee all given pages are mapped
   ...
   */
  /* @nghia_do: Mapping the taken free frame(s) to each page in page table */
  ...
  /* @nghia_do: The taken free frame(s) now has (have) been mapped, so we add it to used_fp_list */
  fpit->fp_next = caller->mram->used_fp_list;
...
int alloc_pages_range(struct pcb_t *caller, int req_pgnum, struct framephy_struct** frm_lst)
{
  ...
  { /* @nghia_do: loop to get free frame and create a free frame list from MEMPHY mram */
